...
const SummaryRow = ({ label, children }: SummaryRowProps) => (
    <div className="flex items-start justify-between gap-3 border-t pt-2.5">
        ...
        <span className="max-w-[62%] truncate text-right text-xs font-semibold">{children}</span>
    </div>
);
...
export const AttendanceSummaryCard = ({
    ...
    const [channel, legalArea, subject, clientReport, preliminaryAnalysis, actions] = useWatch({
        control,
        name: ['channel', 'legalArea', 'subject', 'clientReport', 'preliminaryAnalysis', 'actions'],
    });
        ...
        <section className="grid gap-3.5 rounded-xl border bg-card p-4">
            ...
            <div className="grid gap-2.5">
                ...
                <SummaryRow label="Área jurídica">
                    {legalArea || (
                        <span className="font-normal text-muted-foreground">Não informada</span>
                    )}
                </SummaryRow>
            ...
            <div>
                ...
                {actions.length === 0 ? (
                    <p className="text-xs text-muted-foreground">Nenhuma ação selecionada.</p>
                ) : (
