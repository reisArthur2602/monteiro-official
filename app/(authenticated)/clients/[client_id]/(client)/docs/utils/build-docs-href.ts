...
export const buildClientDocsHref = (clientId: string) => `${buildClientHref(clientId)}/docs`;

/**
 * Monta a URL da listagem preservando apenas os filtros ativos.
 * A página só entra na query string a partir da segunda.
 */
export const buildDocsHref = (clientId: string, params: Partial<ListClientDocumentsParams>) => {
...
export const hasActiveDocsFilters = (params: ListClientDocumentsParams) =>
    Boolean(params.search || params.category || params.visibility || params.status);

/** Endpoint autenticado que faz o streaming do arquivo a partir do FTP. */
export const buildDocumentDownloadHref = (clientId: string, documentId: string) =>
